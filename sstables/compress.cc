...
public:
    ...
    virtual future<temporary_buffer<char>> get() override {
        ...
        const auto ucl = _compression_metadata->uncompressed_chunk_length();
        const uint64_t chunk_base = _pos - _pos % ucl;
        const uint64_t remaining_chunks = (_end_pos - chunk_base + ucl - 1) / ucl;
        // Cap coalescing to the caller's buffer_size (decompressed domain), floor 1 chunk.
        const uint64_t safety_ceiling_chunks = std::max<uint64_t>(1, _buffer_size / ucl);
        ...
        while (true) {
            ...
            if (!produced) {
                ...
                res_units_holder = co_await _permit.request_memory(cap_chunks * ucl);
                out = temporary_buffer<char>(cap_chunks * ucl);
            ...
            auto len = _compression_metadata->get_compressor().uncompress(buf.get(), compressed_len, out.get_write() + produced, ucl);
            produced += len;
            _pos += len - addr.offset;
            _underlying_pos += addr.chunk_len;
            if (_pos >= _end_pos || produced + ucl > out.size() || need_preempt()) {
                break;
            }
        }
        out.trim(produced);
