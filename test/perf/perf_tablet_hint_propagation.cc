...
static
void run_hint_propagation_test(cql_test_env& e, size_t table_count, size_t tables_to_change, bool with_hint) {
    ...
    testlog.info("tablets={} changed={} hint={} time={:.4f}s",
                 table_count, tables_to_change, with_hint ? "yes" : "no", elapsed.count());
}
...
void run_measurement(const bpo::variables_map& opts) {
    ...
    do_with_cql_env_thread([&] (auto& e) {
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            run_hint_propagation_test(e, table_count, tables_to_change, false);
            run_hint_propagation_test(e, table_count, tables_to_change, true);
        }
    }, cfg).get();
}
