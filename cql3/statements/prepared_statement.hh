...
struct cql_metadata_id_type {
    ...
    cql_metadata_id_type() = default;
    explicit cql_metadata_id_type(bytes_view metadata_id) {
    ...
    // For wire protocol: convert to bytes for write_short_bytes
...
template <> struct fmt::formatter<cql3::cql_metadata_id_type> {
    ...
    auto format(const cql3::cql_metadata_id_type& m, fmt::format_context& ctx) const {
        return fmt::format_to(ctx.out(), "{}", m.to_bytes());
    }
};
